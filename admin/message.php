	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="row">
				<div class="col-md-12">
					<?php  
					$select_query = "SELECT * FROM user_message WHERE msg_status = 'unread' ";
					$rslts = $db->select($select_query);

					if ($rslts) {?>
						<h1>Uread Message  list</h1>
								...
								<tbody>
									<?php while ($row = $rslts->fetch_assoc()) { ?>
										<tr>
											...
											<td>
												<a href="?read=<?php echo $row['message_id']; ?>"><button class="btn btn-danger">Mard As Read</button></a>
											</td>
											<td>
...
<?php 
	...
	if (isset($_GET['read'])) {
		...
		$update_qry = "UPDATE user_message SET msg_status = 'read' WHERE message_id = $msg_id ";
		$result = $db->update($update_qry);
		if ($result) {
			echo '<script type="text/javascript">Swal.fire({
					icon: "success",
					title: "Read",
					text: "Mark as read successful",
					})
					setTimeout(function(){
						window.location.href = "message.php";
						}, 2000);
